...
def tool_url(url):
    value = str(url or '')
    return value if value.startswith(('http://', 'https://', '/')) else '/' + value.lstrip('/')


def category_url(slug):
    return f'/Nisulka-Tools/categories/{slug}/'


def page(category, slug, tools):
    title = f'{category} — Free Online Tools | Nisulka Tools'
    desc = f'Explore {len(tools)} free {category.lower()} tools from Nisulka Tools. Fast browser-based utilities for everyday digital work.'
    ...
    for i, t in enumerate(tools, 1):
        ...
        raw = str(t.get('url') or '')
        absolute = raw if raw.startswith(('http://', 'https://')) else BASE.rstrip('/') + '/' + raw.lstrip('/')
    ...
    all_categories = sorted({str(t.get('category') or 'Other Tools') for t in read_tools() if t.get('status') != 'hidden'})
    related = []
    for n in all_categories:
        s = slugify(n)
        if s != slug:
            related.append(f'<a class="category-chip" href="{html.escape(category_url(s), quote=True)}">{html.escape(n)}</a>')
...
for slug, group in groups.items():
    ...
    (target / 'index.html').write_text(page(group['name'], slug, group['tools']), encoding='utf-8')
print(f'Generated {len(groups)} directory-based category pages with only their related tools.')
